...
Url: [http://localhost:8080/travel/h2-console]   
...
some information from its traffic. Retrieve and store the requirement values
below.
...
This information must be exposed in an `/travel/actuator/metrics` API endpoint as JSON.
...
I enabled metrics actuator, so for 
...
call `/travel/actuator/metrics/http.server.requests`

For 
-   Total number of requests resulted in a 4xx response
call `/travel/actuator/metrics/http.server.requests?tag=status:404`
